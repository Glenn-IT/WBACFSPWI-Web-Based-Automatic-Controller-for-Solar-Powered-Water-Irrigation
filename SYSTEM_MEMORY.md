...
│    ├── A1: JSN-SR04T Waterproof Ultrasonic TRIG Output (or HW-080)            │        │
│    ├── A2: Battery Resistor Divider (100kΩ / 33kΩ, Factor: 4.0303)            │        │
│    ├── A3: Solar Resistor Divider (100kΩ / 20kΩ, Factor: 6.0000)              │        │
│    ├── A4: JSN-SR04T Waterproof Ultrasonic ECHO Input (Standing Water Depth) │        │
...
| **HW-080 Surface Water Level (A1)** | `arduino/02_surface_water_level_test/` | • `arduino/07_dual_sensor_pump_integration_test/`<br>• `arduino/08_dc_adapter_presentation_test/`<br>• `arduino/wbacfspwi_arduino_controller/`<br>• `arduino/wbacfspwi_arduino_controller/wiring_guide.html`<br>• `arduino/CALIBRATION_REGISTRY.md`<br>• `arduino/Pinout_and_Schematic.md`<br>• `.agents/rules/arduino_calibration_sync.md` | `HW080_RAW_DRY = 1020`<br>`HW080_RAW_MID = 663`<br>`HW080_RAW_WET = 568`<br>3-point piecewise ruler calibration curve |
...
| **GSM Module SMS Alert System** | `arduino/10_gsm_sms_irrigation_alert_test/` | • `arduino/10_gsm_sms_irrigation_alert_test/10_gsm_sms_irrigation_alert_test.ino`<br>• `arduino/10_gsm_sms_irrigation_alert_test/wiring_guide.html`<br>• `arduino/wbacfspwi_arduino_controller/wiring_guide.html`<br>• `arduino/HARDWARE_STARTUP_GUIDE.md`<br>• `arduino/Pinout_and_Schematic.md`<br>• `SYSTEM_MEMORY.md` | Board: SIM800L / SIM900 GSM/GPRS Module<br>Pins: Arduino D2 (RX) <- GSM TXD; Arduino D3 (TX) -> GSM RXD via 1k/2k divider<br>Power: 3.7V - 4.4V (4.0V nominal, 2A burst) + 1000µF buffer cap<br>Recipients: Primary `+639158127228`, Secondary `+639242074903`<br>Automated SMS triggers: Irrigation Started (<40%), Stopped (>=50%), and Restarted (<40% again) |
| **JSN-SR04T Waterproof Ultrasonic Sensor** | `arduino/11_jsn_sr04t_ultrasonic_water_level_test/` | • `arduino/11_jsn_sr04t_ultrasonic_water_level_test/11_jsn_sr04t_ultrasonic_water_level_test.ino`<br>• `arduino/10_gsm_sms_irrigation_alert_test/10_gsm_sms_irrigation_alert_test.ino`<br>• `arduino/wbacfspwi_arduino_controller/`<br>• `arduino/11_jsn_sr04t_ultrasonic_water_level_test/wiring_guide.html`<br>• `arduino/11_jsn_sr04t_ultrasonic_water_level_test/README.md`<br>• `arduino/HW080_DIAGNOSTICS_AND_JSN_SR04T_GUIDE.md`<br>• `arduino/CALIBRATION_REGISTRY.md`<br>• `arduino/Pinout_and_Schematic.md`<br>• `SYSTEM_MEMORY.md` | Board: JSN-SR04T v2.0/v3.0<br>Pins: Arduino Pin A1 (TRIG output), Pin A4 (ECHO input)<br>Power: LM2596 Star 5V Rail -> 5V; Star GND -> GND<br>Live Calibrated Geometry: `sensorClearanceCM = 20.0cm`, `containerDepthCM = 8.0cm`<br>Soil Bed Distance (0% Empty): `28.0cm`<br>50.0% Target: `24.0cm` (Pump OFF)<br>40.0% Refill: `24.8cm` (Pump ON)<br>100% Full: `20.0cm` |
...
| **Admin Navigation & Pages** | `public/admin/` (adding/modifying pages) | • `public/admin/partials/sidebar.php`<br>• `$activePage` variable on target page<br>• User role permission check in `guard.php` or top of file | `$navItems` keys and labels: `dashboard`, `telemetry`, `logs`, `reports`, `users`, `profile` |
...
| **A1** | JSN-SR04T Ultrasonic TRIG Output (or HW-080) | Digital OUT / Analog IN | 10µs trigger pulse (5V TTL). Legacy HW-080: 0.0V–5.0V (Raw: 1020/663/568). |
...
| **A4** | JSN-SR04T Ultrasonic ECHO Input | Digital IN | 5V TTL echo pulse duration proportional to distance (35ms timeout). |
...
1. **Layer 1: Hysteresis Band**
   - **PUMP START (ON):** Surface Water Level drops $< 40.0\%$ (and Battery $\ge 10.00\text{V}$).
   - **PUMP STOP (OFF):** Surface Water Level reaches $\ge 50.0\%$.
   - **Deadband / Buffer:** $40.0\% \le \text{Water Level} < 50.0\%$ maintains previous state (no rapid cycling).
...
3. **Layer 3: Wave Settling Window (`SETTLING_DELAY_MS = 10000ms`)**
   ...
   - If water settles $\ge 40.0\%$, pump stays shut off (STABLE / BUFFER ZONE). Pump only refills if settled level drops below the refill trigger ($< 40.0\%$).
